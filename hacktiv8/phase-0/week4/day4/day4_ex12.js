...
function countProfit(shoppers) {
    ...
    for (var i = 0; i < listBarang.length; i++) {
        ...
        for (var j = 0; j < shoppers.length; j++) {
            if (shoppers[j].product === listBarang[i][0] && shoppers[j].amount <= listProfit[i].leftOver) {
                listProfit[i].shoppers.push(shoppers[j].name)
                listProfit[i].totalProfit += listBarang[i][1] * shoppers[j].amount
                listProfit[i].leftOver -= shoppers[j].amount
            }
        }
        return listProfit
    }
    // console.log(listProfit);liat isi dari profit
}
