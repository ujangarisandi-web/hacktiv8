...
function deepSum(arr) {
    var sum = 0
    for (var i = 0; i < arr.length; i++) { // array pertama
        for (var j = 0; j < arr[i].length; j++) { //array kedua
            for (var k = 0; k < arr[i][j].length; k++) { //Array ke tiga 
                sum += arr[i][j][k]
            }
        }
    }
    if (arr.length === 0) { // untuk data array yang kosong
        return 'No number'
    }
    return sum
}
